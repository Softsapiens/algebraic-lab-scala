...
class FunProfun extends FlatSpec with Matchers {
  ...
  object newoptics1 {
    ...
    From packages:
      ...
      https://github.com/purescript-contrib/purescript-profunctor-lenses/blob/master/src/Data/Lens/Lens.purs
    */
    def lens[S, T, A, B](getter: S => A, setter: (S, B) => T): Lens[Function1, S, T, A, B] =
      new Optic[Strong, Function1, S, T, A, B] {
        def apply(ex: Strong[Function1])(pab: Function1[A, B]): Function1[S, T] =
          ex.dimap[S, T, (A, B => T), (B, B => T)](s => (getter(s), b => setter(s, b)), { case (b, f) => f(b) })(ex.first[B => T, A, B](pab))
      }
    ...
    object getter {
      ...
      def to[R, S, T, A, B](f: S => A): Fold[R, S, T, A, B] =
        new Optic[Id, Forget[R, ?, ?], S, T, A, B] {
          // P[A, B] => P[S, T]
          override def apply(ex: profunctor.Id[Forget[R, ?, ?]])(p: Forget[R, A, B]): Forget[R, S, T] = f andThen p
        }
    }
...
*/
  ...
  "newoptics1 -->  Testing Lens composition" should "work" in {
    import newoptics1._
    import profunctor.instances._
    import getter._
    ...
    val _lpAge = lens[Person, Person, Age, Age](_.age, { case (p, v) => p.copy(age = v) })
    ...
    val _pgtrYear: Getter[Person, Person, Int, Int] = getter.to({ ( (p:Person) => p.age) andThen(_.years) })

    assert(view(_gtrYear, new Age(40)) === 40)
